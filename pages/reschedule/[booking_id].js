...
export default function Reschedule() {
  ...
  const h = DateTime.now().hour; // Use Luxon's DateTime to get the current hour

  const mm = DateTime.now().minute; // Use Luxon's DateTime to get the current minute
  const half = mm > 29;
              ...
              <div className="section-selectBooking flex-column" id="content">
                <div className="d-flex align-items-end date-card-container">
                  {[...Array(100)].map((elementInArray, index) => {
                    const targetDate = DateTime.now().plus({ days: index });
                    const day_ = targetDate.toFormat('cccc');
                    const day = day_.substring(0, 3);
                    const d = targetDate.toFormat('dd');
                    const m = targetDate.toFormat('MMMM');
